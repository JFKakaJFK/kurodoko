...
const cellWidth = (width, cols, gap) => (width - (cols - 1) * gap) / cols
const calcHeight = (width, rows, cols, gap) => cellWidth(width, cols, gap) * rows + (rows - 1) * gap
...
const GridStyles = styled.div`
  ...
  > div {
    ...
    ${props => props.width && `height: ${calcHeight(props.width, props.rows, props.cols, props.theme[props.type].gridGap)}px;`}
    /* ${props => props.width && `height: ${calcGridHeight(props.width, props.rows, props.cols, props.theme, props.type)}px;`} */
    ...
    grid-template-rows: ${({ rows, theme, type }) => `repeat(${rows}, 1fr)`};
    grid-template-columns: ${({ cols, theme, type }) => `repeat(${cols}, 1fr)`};
    ...
    font-size: ${props => cellWidth(props.width || '16', props.cols, props.theme[props.type].gridGap) / 2}px; /* * (2 / 3) */
...
function Grid(props) {
  ...
  return (
    <GridStyles ref={gridRef} rows={props.rows} cols={props.cols} width={width} type={props.type}>
      <div >
        {props.children}
      </div>
    </GridStyles>
